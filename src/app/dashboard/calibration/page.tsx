...
export default function CalibrationPage() {
  ...
  return (
    ...
    <div className="pb-28">
      ...
      <section className="mb-6 opacity-0 animate-[fadeSlideUp_0.5s_ease_0.1s_forwards]">
        ...
        <Card variant="default" padding="none" className="overflow-hidden border border-gray-100/80 shadow-[0_2px_20px_rgba(0,0,0,0.05)] rounded-3xl">
          <div className="flex flex-col lg:flex-row">
            {/* Left: Camera preview + target selector */}
              ...
              <div className="relative w-full aspect-video rounded-2xl overflow-hidden bg-slate-900 mb-5 shadow-sm border border-slate-800">
                <div className="absolute inset-0 flex items-center justify-center">
                  <div className="text-center">
                    ...
                    <p className="text-white/40 text-xs font-medium tracking-wide">LIVE FEED</p>
                  ...
                  return (
                    <button
                      ...
                      className={`
                        ...
                        ${isActive ? "border-lime-500 bg-lime-50/50 shadow-sm" : "border-gray-100 bg-gray-50/50 hover:border-gray-200 hover:bg-white"}
                      `}
                    >
                      <div className="relative flex-shrink-0">
                        <div className="w-10 h-10 rounded-xl shadow-sm border border-white" style={{ backgroundColor: previewColor }} />
                        {isActive && (
                      ...
                      <div className="text-left flex-1 min-w-0">
                        <div className="flex items-center gap-1.5">
                          ...
                          <p className={`text-sm font-semibold capitalize ${isActive ? "text-lime-800" : "text-gray-700"}`}>{target}</p>
                        </div>
                      ...
                      <div
                        ...
                        style={{ background: `linear-gradient(to bottom, ${hsvToRgb(config[target].hueMin, 200, 180)}, ${hsvToRgb(config[target].hueMax, 200, 180)})` }}
                      />
                    </button>
                  );
                })}
            ...
            <div className="flex-1 p-6">
              <div className="flex items-center justify-between mb-5">
                <div>
                  ...
                  <p className="text-xs text-gray-500 mt-0.5">Hue · Saturation · Value thresholds</p>
                </div>
                <div className="w-6 h-6 rounded-lg shadow-sm border border-white" style={{ backgroundColor: activeColor === "larva" ? larvaColor : prepupaColor }} />
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  />
                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide w-12 text-right">Hue</span>
                </div>
                <div className="space-y-4">
                  <SliderControl label="Hue Min" value={activeParams.hueMin} min={0} max={180} onChange={(v) => updateHSV(activeColor, "hueMin", v)} colorHint={hsvToRgb(activeParams.hueMin, 200, 180)} />
                  <SliderControl label="Hue Max" value={activeParams.hueMax} min={0} max={180} onChange={(v) => updateHSV(activeColor, "hueMax", v)} colorHint={hsvToRgb(activeParams.hueMax, 200, 180)} />
                </div>
              </div>
              ...
              <div className="mb-6">
                <div className="flex items-center gap-2 mb-3">
                  ...
                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Saturation</span>
                  <div className="flex-1 h-px bg-gray-200" />
                </div>
                <div className="space-y-4">
                  <SliderControl label="Sat Min" value={activeParams.satMin} min={0} max={255} onChange={(v) => updateHSV(activeColor, "satMin", v)} />
                  <SliderControl label="Sat Max" value={activeParams.satMax} min={0} max={255} onChange={(v) => updateHSV(activeColor, "satMax", v)} />
                </div>
              </div>
              ...
              <div>
                <div className="flex items-center gap-2 mb-3">
                  <div className="flex-1 h-px bg-gray-200" />
                  <span className="text-[10px] font-semibold text-gray-400 uppercase tracking-wide">Value (Brightness)</span>
                  <div className="flex-1 h-px bg-gray-200" />
                </div>
                <div className="space-y-4">
                  <SliderControl label="Val Min" value={activeParams.valMin} min={0} max={255} onChange={(v) => updateHSV(activeColor, "valMin", v)} />
                  <SliderControl label="Val Max" value={activeParams.valMax} min={0} max={255} onChange={(v) => updateHSV(activeColor, "valMax", v)} />
                </div>
              </div>
            </div>
          </div>
      ...
      {/* left disesuaikan dengan pl-24 (96px) dari DashboardLayout */}
